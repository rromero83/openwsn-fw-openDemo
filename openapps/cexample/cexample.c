...
owerror_t cexample_receive(OpenQueueEntry_t* msg, coap_header_iht* coap_header,
...
void cexample_init() {
	...
	cexample_vars.timerId = opentimers_start(CEXAMPLEPERIOD, TIMER_PERIODIC,
			TIME_MS, cexample_timer_cb);
}
...
void cexample_task_cb() {
	...
	if (pkt == NULL) {
		openserial_printError(COMPONENT_CEXAMPLE, ERR_NO_FREE_PACKET_BUFFER,
