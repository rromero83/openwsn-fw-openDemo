...
static const uint8_t dst_addr[] = { 0xbb, 0xbb, 0x00, 0x00, 0x00, 0x00, 0x00,
...
owerror_t cstorm_receive(OpenQueueEntry_t* msg, coap_header_iht* coap_header,
...
void cstorm_task_cb() {
	...
	pkt->payload[0] = (COAP_OPTION_NUM_URIPATH - 7) << 4
			| (sizeof(cstorm_path0) - 1);
	numOptions++;
	...
	// send
	outcome = opencoap_send(pkt, COAP_TYPE_NON, COAP_CODE_REQ_PUT, numOptions,
			&cstorm_vars.desc);
